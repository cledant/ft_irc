...
/*   Updated: 2017/03/29 18:58:45 by cledant          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
							...
							const t_err err)
{
	...
	else if (err == ERR_USER_ALREADY_IN_CHAN)
		ft_strcat(cmd->cmd, "SMSG Already in that channel !");
	else
							...
							t_env *env, const int fd_sock)
{
	...
	if (ft_is_str_alphanum(chan_name + 1) == 0 ||
		...
		return (set_error(cmd, fd_sock, ERR_CHAN_NAME));
	if ((cmd->id_chan = srv_join_user_to_channel(env, fd_sock, chan_name))
			== -1)
		return (set_error(cmd, fd_sock, ERR_MAX_CHAN));
	if (cmd->id_chan == -2)
		return (set_error(cmd, fd_sock, ERR_USER_ALREADY_IN_CHAN));
	...
	printf("%s : Client ID : %d joined a channel !\n", env->file_name, fd_sock);
	return (1);
}
